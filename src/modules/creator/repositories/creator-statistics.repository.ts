...
export class CreatorStatisticsRepository extends Repository<CreatorStatisticsEntity> {
  ...
  async saveStatistics(
    ...
    // platformStats and categoryStats are handled by separate entities
    if (statisticsData.monthlyAverageViews !== undefined) {
      statistics.monthlyAverageViews = statisticsData.monthlyAverageViews;
    }
    if (statisticsData.contentQualityScore !== undefined) {
  ...
  async getCreatorsByGrowthRate(
    limit = 10,
    type: 'followers' | 'content' = 'followers'
  ): Promise<CreatorStatisticsEntity[]> {
  ...
  async getStatisticsOverview(): Promise<{
    ...
    const [totalCreators, aggregateStats, activeCreators] = await Promise.all([
      this.count(),
      this.createQueryBuilder('stats')
        .select([
          'SUM(stats.totalFollowers)',
          'totalFollowers',
          'SUM(stats.totalContent)',
          'totalContent',
          'SUM(stats.totalViews)',
          'totalViews',
          'AVG(stats.averageEngagementRate)',
  ...
  > {
    const result = await this.createQueryBuilder('stats')
      .select([
        'JSON_UNQUOTE(JSON_KEYS(stats.categoryStats))',
        'categories',
        'COUNT(*)',
        'creatorCount',
      ])
  ...
  async getGrowthTrends(days = 30): Promise<
    Array<{
      date: string;
      newCreators: number;
