    ...
    try {
      ...
      for (const platform of youtubePlatforms) {
        try {
          ...
          const videoSyncStatus = syncEntity?.videoSyncStatus || VideoSyncStatus.NEVER_SYNCED;

          // 동기화 상태에 따른 분기 처리
          switch (videoSyncStatus) {
            case VideoSyncStatus.NEVER_SYNCED:
            case VideoSyncStatus.FAILED:
              syncedCount = await this.performFullSync(platform);
              break;

            case VideoSyncStatus.COMPLETED:
              syncedCount = await this.performIncrementalSync(platform);
              break;

            case VideoSyncStatus.IN_PROGRESS:
