...
export class QuotaMonitorService {
  ...
  async getDailyQuotaUsage(
    ...
    try {
      ...
      const usages = await this.quotaUsageRepo.findByProviderAndDateRange(
        apiProvider,
        startDate,
        endDate
      );
  ...
  > {
    try {
      ...
      const usages = await this.quotaUsageRepo.findByProviderAndWeekRange(
        apiProvider,
        startDate,
        endDate
      );
  ...
  async cleanupOldQuotaRecords(): Promise<{ deletedCount: number }> {
    try {
      ...
      const { deletedCount } = await this.quotaUsageRepo.deleteOldRecords(thirtyDaysAgo);
