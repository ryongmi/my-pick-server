...
1. **권한 검증**: 모든 사용자 대상 API에 `@UseGuards(AuthGuard)` 필수
2. **TCP 연동**: auth-server, authz-server가 실행 중이어야 정상 동작
3. **데이터베이스**: 마이그레이션 스크립트는 수동 실행 필요
4. **외부 API**: YouTube, Twitter API 키 설정 필수 (콘텐츠 동기화용)
5. **캐싱**: Redis 연결 실패 시 서비스 시작 불가
